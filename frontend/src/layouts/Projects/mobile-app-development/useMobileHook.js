...
const useMobileHook = (reduxState, reduxActions) => {
    const { id } = useParams()
    ...
    async function reloadAllData() {
        if (handleRole().projectManager || handleRole().admin) { getMobileAppDevList() }
    ...
    const projectForReview = async () => {
        setSendChanges(true)
        await apiClient.get('/api/mobile-app/for-review/' + id)
            .then(({ data }) => {
    ...
    const projectOngoing = async () => {
        setSendChanges(true)
        await apiClient.get('/api/mobile-app/ongoing/' + id)
            .then(({ data }) => {
    ...
    const withRevision = async () => {
        setSendChanges(true)
        await apiClient.get('/api/mobile-app/with-revision/' + id)
            .then(({ data }) => {
