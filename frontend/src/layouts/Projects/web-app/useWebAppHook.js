...
const useWebAppHook = (reduxState, reduxActions) => {
    const { id } = useParams()
    ...
    async function reloadAllData() {
        if (handleRole().projectManager || handleRole().admin) { getWebAppDevList() }
    ...
    const projectForReview = async () => {
        setSendChanges(true)
        await apiClient.get('/api/web-app/for-review/' + id)
            .then(({ data }) => {
    ...
    const projectOngoing = async () => {
        setSendChanges(true)
        await apiClient.get('/api/web-app/ongoing/' + id)
            .then(({ data }) => {
    ...
    const withRevision = async () => {
        setSendChanges(true)
        await apiClient.get('/api/web-app/with-revision/' + id)
            .then(({ data }) => {
