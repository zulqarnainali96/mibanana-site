...
const Chating = ({ reduxState, reduxActions }) => {
    ...
    const personProject = () => {
        if (reduxState.project_list?.CustomerProjects) {
            const singleProject = reduxState.project_list?.CustomerProjects?.find(item => item?._id === id)
    ...
    const onSendMessage = async (event) => {
        ...
        await apiClient.put("/chat-message", data)
            .then(({ data }) => {
                // setIsComingMsg(true)
        ...
        await apiClient.post('/api/send-message-to-others', userData).then(({ data }) => {
            console.log(data)
        })
    ...
    const getChatMessage = async () => {
        await apiClient.get('/chat-message/' + id)
            .then(({ data }) => {
                ...
                <Grid container spacing={2} height={'90vh'} paddingInline={"25px"}>
                    <Grid item xxl={6} xl={6} lg={6} md={12} xs={12}>
                        <MDBox {...styleProps} sx={{ overflowY: "scroll" }}>
                            <MDTypography sx={({ palette: { primary } }) => ({
                                backgroundColor: primary.main,
                                padding: '10px',
                                borderTopLeftRadius: '4px',
                                borderTopRightRadius: '6px',
                            })}
                                variant="h4" pb={1}>{personProject()?.project_title ? personProject().project_title : ''}</MDTypography>
                            <Grid container justifyContent={"space-between"} spacing={4} padding={2} alignItems={"center"}>
                                <Grid item xxl={6} height={personProject()?.team_members?.length > 0 && isManager ? "187px" : undefined}>
                                    <MDTypography variant="h4" fontSize="medium">Team Members</MDTypography>
                                    ...
                                    <br />
                                    {
                                        personProject().team_members?.length > 0 ?
                                            personProject().team_members?.map((item, i) => <TeamMembers key={i} team_member={item} isManager={isManager} setIsMember={setIsMember} openSuccessSB={openSuccessSB} openErrorSB={openErrorSB} setRespMessage={setRespMessage} />
                                            ) : null
                                    }
                                    {isManager ?
                                        <SelectMembers
                                ...
                                <Grid item xxl={6} sx={(role?.customer ? {alignSelf: 'end',paddingTop: '1px',paddingBottom: '14px'} : role?.admin ? {paddingBottom:'16px',alignSelf:'self-end'} : {})} >
                                    <MDTypography variant="h4">Active</MDTypography>
                                    <MDTypography variant="h6" fontSize="small" sx={{color : '#4a4747',fontWeight:300}}>{!personProject()?.is_active ? "This Project is not active" : "Project is Active"}</MDTypography>
                                    {isManager ? (<FormControlLabel required control={<Checkbox checked={isActive}
                                    ...
                                    />} label={`${!personProject()?.is_active ? 'Make Project Active' : 'Active'}`} />) : null}
                                ...
                                <Grid item xxl={3} alignSelf={personProject().team_members?.length > 0 ? "center" : "baseline"}>
                                    <MDTypography variant="h4">Project Category</MDTypography>
                                    <MDTypography variant="h6" sx={{color : '#4a4747',fontWeight:300}}>{personProject()?.project_category ? personProject().project_category : ''}</MDTypography>
                                </Grid>
                                <Grid item xxl={3}>
                                    <MDTypography variant="h4">Design Type</MDTypography>
                                    <MDTypography variant="h6" sx={{color : '#4a4747',fontWeight:300}}>{personProject()?.design_type ? personProject()?.design_type : ''}</MDTypography>
                                </Grid>
                                <Grid item xxl={3}>
                                    <MDTypography variant="h4">Brand</MDTypography>
                                    <MDTypography variant="h6" sx={{color : '#4a4747',fontWeight:300}}>{personProject()?.brand ? personProject().brand : ''}</MDTypography>
                                </Grid>
                                <Grid item xxl={3}>
                                    <MDTypography variant="h4">Sizes</MDTypography>
                                    <MDTypography variant="h6" sx={{color : '#4a4747',fontWeight:300}}>{personProject()?.sizes ? personProject().sizes : 'Not Provied'}</MDTypography>
                                </Grid>
                                <Grid item xxl={12}>
                                    <MDTypography variant="h4">Project Description</MDTypography>
                                    <MDTypography variant="h6" sx={{color : '#4a4747',fontWeight:300}}>{personProject()?.project_description ? personProject().project_description : "No description"}</MDTypography>
                                </Grid>
                                ...
                                {isManager ?
                                    <Grid item xxl={12} alignSelf={"flex-end"} textAlign={"right"}>
                                        <MDButton disabled={loading} endIcon={<MoonLoader loading={loading} size={18} color='#fff' />} color="dark" bgColor="success" type="button" onClick={SubmitProject}>
                                            Submit
                                        </MDButton>
                                    </Grid>
                                    : null}
                                    ...
                                    <MDBox>
                                        <Grid
                                            ...
                                            <Grid item xxl={12} xl={12} lg={12} width={"100%"} sx={{ overflowY: "auto" }}>
                                                <MDBox {...stylesProps2.sx2} sx={stylesProps2.sx} p={1} ref={chatContainerRef}>

                                                    <div className="chat" >
                                                        {msgArray?.length ? msgArray.map((item, index, messages) => {
                                                            return (
                                                                <pre
                                                                    key={index}
                                                                ...
                                                                >
                                                                    <div className="user-name" style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                                                                        ...
                                                                        <p className='ff' style={{ fontSize: "11px" }}>{item.name}<span style={{ fontSize: "11px" }}>{" (" + item?.role + ")"}</span></p>
                                                                    </div>
                                                                    <div className="message-content">{item.message}</div>
                                                                    <span className='ff' style={{ fontSize: '9px', fontWeight: '400', color: '#444' }}>{item.time_data ? item.time_data : null}</span>
                                                                </pre>
                                                            )
                                                        }
                                                        ) : null}
                                            ...
                                            <Grid item xxl={12} xl={12} lg={12} md={12} width={"100%"}>
                                                <MDBox height={"6vh"} width="100%" p={1}
                                                    sx={{
                    ...
                    <Grid item xxl={6} xl={6} lg={6} md={12} xs={12}>
                        ...
                        <FileUploadContainer setRespMessage={setRespMessage} openSuccessSB={openSuccessSB} openErrorSB={openErrorSB} />
                    </Grid>
                    {renderSuccessSB}
                </Grid >
            </MDBox >
