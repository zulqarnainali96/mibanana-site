...
const FileUploadContainer = ({ openErrorSB, openSuccessSB, setRespMessage, reduxState, reduxActions }) => {
    ...
    const deleteCustomerFiles = async (filename) => {
        setFileLoading(true)
        await apiClient.delete(`/api/del-customer-files/${id}/${filename}`).then(({ data }) => {
            const { message } = data
            setRespMessage(message)
            setTimeout(() => {
                setFileLoading(false)
    ...
    const deleteFile = async (filename) => {
        ...
        else { deleteCustomerFiles(filename) }
    }
    ...
    return (
        <MDBox className={classes.Container}>
            <Grid container className={classes.uploadBox}>
                <Grid item xxl={8} xl={8} lg={8} md={8} xs={8} >
                    <MDBox className={classes.fileNameContainer} >
                        <MDInput type="text" sx={inputSxStyles} value={currentImage.name} className={classes.fileNameInput} />
            ...
            <MDBox className={classes.sideBarContainer}>
                ...
                {fileMsg ? <p style={{ color: 'white', fontSize: '16px', fontWeight: '600' }}>{fileMsg}</p> : null}
            ...
            <MDBox className={classes.secondContainer}>
                <MDTypography fontSize="medium" variant="h5">YOUR ARTWORK</MDTypography>
