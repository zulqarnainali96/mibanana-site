...
const SelectMembers = ({
    ...
    useEffect(() => {
        if (manager) {
            apiClient.get("/api/get-designer-list/" + id)
                .then(({ data }) => {
                    setDesignerObj(data?.designerlist)
                    if (existTeamList?.length > 0) {
                        let currentTeam = data?.designerlist.filter((item, i) => item._id !== existTeamList[i]._id)
    ...
    return (
        ...
        <>
            <FormControl sx={{
                m: 1, height: '65px', "& .MuiOutlinedInput-root": {
                    minWidth: '272px',
                ...
                <Select
                    ...
                    multiple
                    sx={{width: '100%'}}
                    IconComponent={() => memberName?.length ?
                        <CloseSharp
                            fontSize='small'
