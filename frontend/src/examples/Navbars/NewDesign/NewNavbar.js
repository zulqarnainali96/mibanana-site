...
const NewNavbar = ({ reduxState, reduxActions, routes }) => {
  ...
  async function updateAllChatMessage() {
    ...
    if (getMessageNotification()) {
      await apiClient
        .get(`/api/udpate-all-notifications/${id}`)
        .then(({ data }) => {
  ...
  return (
    <div>
      ...
      <Grid
        container
        className={navbarStyles.gridContainer}
        sx={{ paddingInline: !is600 ? "0.5rem" : "5rem", alignItems: !is800 && "center" }}
      >
        <Grid item xxl={6.5} xl={4} lg={4} md={5} sm={6} xs={5} textAlign={!isLarge && "center"}>
          <img
            src={MibananaIcon}
            loading="lazy"
            width={!is600 ? "190px" : "250px"}
            height={!is600 ? "56px" : "54px"}
          />
        </Grid>
        <Grid item xxl={5.5} xl={8} lg={8} md={7} sm={6} xs={7}>
          <Grid container alignItems="center">
            <Grid
              ...
              sm={12}
              xs={12}
              sx={gridItemResponsive}
            >
              <img
                src={personImage}
                style={{ display: "block" }}
                width={!is600 ? "45px" : !is800 ? "50px" : "61px"}
                height={!is600 ? "45px" : !is800 ? "50px" : "61px"}
              />
              <div>
                <MDTypography className={navbarStyles.insideText} sx={responsiveStyle}>
                  Hello {showRoles()}!
                </MDTypography>
                <MDTypography
                  fontSize="medium"
                  className={`${navbarStyles.poppins} ${navbarStyles.userRole}`}
                  sx={roleResponsive}
                >
                  {showPersonRoles()}
                </MDTypography>
      ...
      <div className="headersidebardiv">
        {" "}
